...
% 2026-09-22：优先 G0（几 Pa 自由碎石—细颗粒床），G1（稳定硬岩）为后备；
% 0.1/1/10/100 N 仅为首批作业力探索档，力矩、持续时间与资源上限待冻结。
\planned{Prioritize extremely weak regolith without a pre-existing fixed rock
support (G0), using exploratory task forces of 0.1, 1, 10, and 100 N.
Treat accessible, demonstrably stable rock (G1) as a fallback if G0 cannot meet
the specified task and resource limits. Distinguish cohesion, inferred effective
strength, and calibrated anchor resistance; specify torque, duration, permitted
motion, and resource limits before ranking configurations.}
